    ...
    def step(self):
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                p.data.add_(g, alpha=-lr*scale) # actual weight update: weights = weights - lr*scale * orthogonalized_gradient

# -----------------------------------------------------------------------------
# PyTorch nn.Module definitions for the GPT-2 model

# Rotary class to rotate Q, K for position awareness
...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.c_fc = nn.Linear(config.n_embd, 4 * config.n_embd, bias = False) # 768 → 3072
        self.c_proj = nn.Linear(4 * config.n_embd, config.n_embd, bias = False) # 3072 → 768
...
# init the optimizers
# optimizer1: AdamW for the embedding / lm_head (1D params, not suited for Muon)
# optimizer2: Muon for all transformer block weight matrices (2D params)
...
for step in range(args.num_iterations + 1):
    ...
    # ignore first 10 steps for timing (following Keller Jordan)
    if step == 10:
        training_time_ms = 0
        t0 = time.time()
