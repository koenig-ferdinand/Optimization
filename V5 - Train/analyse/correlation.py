...
def compute_correlations(iterations, val_loss, arrays):
    ...
    for opt in OPTIMIZERS:
        loss_aligned = _align_loss(*val_loss[opt], iterations)
...
def plot_correlation_heatmaps(corr, mat_type):
    ...
    out  = os.path.join(OUTPUT_DIR, f'correlation_heatmap_{safe}.png')
...
def plot_scatter_top_pairs(corr, iterations, val_loss, arrays, n_top=16):
    ...
    for idx, (score, metric, mat, layer, mi) in enumerate(top):
        ...
        for opt in OPTIMIZERS:
            ...
            if arr is None:
                continue
            x    = arr[layer].astype(float)
            y    = _align_loss(*val_loss[opt], iterations)
            mask = np.isfinite(x) & np.isfinite(y)
    ...
    fig.tight_layout()
    out = os.path.join(OUTPUT_DIR, 'correlation_top_scatter.png')
    fig.savefig(out, dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f'  → {out}')
...
def plot_metric_summary(corr):
    ...
    out = os.path.join(OUTPUT_DIR, 'correlation_summary.png')
    fig.savefig(out, dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f'  → {out}')
