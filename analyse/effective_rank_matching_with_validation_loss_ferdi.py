...
def process_step(step):
    data = torch.load(f'data/muon/state_step{step:06d}.pt', map_location='cpu', weights_only=False)
    model_muon = data['model']
    data = torch.load(f'data/adamw/state_step{step:06d}.pt', map_location='cpu', weights_only=False)
...
# run in parallel
all_step_data = Parallel(n_jobs=4)(delayed(process_step)(step) for step in iterations)
...
for i, mat in enumerate(matrix_types):
    ...
    im = ax_heat.imshow(diff, aspect='auto', cmap='inferno_r', origin='lower', vmin=0, vmax=300)
...
plt.tight_layout()
plt.savefig('analyse/plots/old_eff_heatmap_combined_convergence_curve.png', dpi=300, bbox_inches='tight')
plt.close()
