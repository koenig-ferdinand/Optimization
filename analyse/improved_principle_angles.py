...
# window indeces
def window_size(S, i, tolerance= 0.05): 
    lower, upper = i, i
    j = i
    while(j>=0 and S[j] <= S[i] + tolerance*S[i]):
        lower = j
        j -= 1

    k = i
    while(k < len(S) and S[k] >= S[i] - tolerance*S[i]): 
        upper = k
        k += 1
...
def boundary(X, Y): 
    ...
    k = int(min(functions.energy_k(S_X), functions.energy_k(S_Y)))
