...
data = torch.load(f'data/muon/state_step000000.pt', map_location = 'cpu')
model_muon = data['model']
data = torch.load(f'data/adamw/state_step000000.pt', map_location = 'cpu')
...
for m, metric in enumerate(metrics): 
    ...
    plt.tight_layout()
    plt.savefig(f'analyse/midtraining/0_{metric}.png', dpi=600)
    plt.close()
